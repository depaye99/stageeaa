...
export default function NewEvaluationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Ici vous pourriez sauvegarder l'évaluation en base
      const evaluationData = {
        stagiaire_id: stagiaireId,
        tuteur_id: user.id,
        competences: competences,
        commentaire: commentaire,
        note_globale: competences.reduce((acc, comp) => acc + comp.note, 0) / competences.length
      }

      // Simulation de sauvegarde
      console.log("Évaluation:", evaluationData)
                        ...
                        <SelectContent>
                          {stagiaires.map((stagiaire) => (
                            <SelectItem key={stagiaire.id} value={stagiaire.id}>
                              {stagiaire.users?.name || "Nom inconnu"}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
