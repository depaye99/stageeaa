...
-- Policies for stagiaires
CREATE POLICY "Stagiaires can view their own data" ON stagiaires
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Tuteurs can view their stagiaires" ON stagiaires
  FOR SELECT USING (auth.uid() = tuteur_id);

-- Policies for demandes
CREATE POLICY "Users can view their own demandes" ON demandes
  FOR SELECT USING (
    auth.uid() IN (
      SELECT user_id FROM stagiaires WHERE id = stagiaire_id
    ) OR auth.uid() = tuteur_id
  );
